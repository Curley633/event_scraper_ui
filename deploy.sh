###################################################################
##  This script orchestrates the deployment to the production VM ##
    ...
    then
        if [[ -z $2 ]]
            then
                ...
                ssh james@206.189.165.104 "cd var/www/event_scraper_ui && time docker-compose -f docker-compose-prod.yml up -d --build && docker system prune --force -a && docker volume prune --force && sudo service jenkins restart && echo 'done' " || exit 1

                ssh james@206.189.165.104 "sudo service jenkins restart && echo 'done' " || exit 1
